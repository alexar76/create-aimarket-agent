...
La variante TypeScript génère un fournisseur `node:http` **sans dépendance d'exécution** et avec le
même `capability.json`. Les deux variantes signent des enveloppes identiques octet pour octet : un
consommateur écrit dans l'un ou l'autre langage vérifie l'un ou l'autre fournisseur — un test de
parité entre langages, dans le paquet npm, échoue si un seul octet diverge. Voir la
[documentation](typescript.fr.md).

## Tutoriel complet : créer un agent utile

Le tutoriel transforme le squelette en **THEMIS**, un agent réel pour les achats
et la sécurité avec politique déterministe, calcul des coûts, correspondances OWASP Agentic,
vérification Metis asynchrone et différée, et résultats signés.

[Ouvrir le tutoriel en français](tutorials/themis.fr.md) ·
[voir le dépôt final](https://github.com/alexar76/themis)
