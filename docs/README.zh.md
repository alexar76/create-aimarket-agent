...
TypeScript 版本会生成一个基于 `node:http`、**零运行时依赖**的提供方，并使用完全相同的
`capability.json`。两种版本签出的信封逐字节一致，因此任一语言编写的消费方都能验证任一提供方——
npm 包中的跨语言一致性测试只要发现一个字节的差异就会失败。参见[文档](typescript.zh.md)。

## 完整教程：构建一个实用智能体

教程会把基础模板扩展为 **THEMIS**：一个真实的采购与安全智能体，包含
确定性策略、成本计算、OWASP Agentic 风险映射、延迟异步 Metis 验证，以及带签名的结果。

[打开中文教程](tutorials/themis.zh.md) ·
[查看完整代码仓库](https://github.com/alexar76/themis)
