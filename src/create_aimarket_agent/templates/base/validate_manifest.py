...
def validate_invoke_url(raw: object) -> None:
    ...
    try:
        ...
        _ = url.port  # Force validation of malformed/out-of-range ports.
...
def main() -> int:
    ...
    finite_number(data, "price_per_call_usd", minimum=0.0, maximum=1000.0)
    if "p50_latency_ms" in data:
        finite_number(data, "p50_latency_ms", minimum=0.0, maximum=86_400_000.0)
    if "success_rate_30d" in data:
        finite_number(data, "success_rate_30d", minimum=0.0, maximum=1.0)
    ...
    for key in ("input_schema", "output_schema"):
        ...
        if schema.get("type") != "object":
            fail(f"{key}.type must be object")
